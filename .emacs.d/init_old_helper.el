...
;;Kluczowe elementy:
;;propertize — nadaje kolor/styl tekstowi
;;(:eval ...) — wykonuje kod dynamicznie przy każdym odświeżeniu
;;%l:%c — linia i kolumna
;;%m — major mode
;;buffer-modified-p — czy bufor ma niezapisane zmiany
;;Możesz wrzucić tam cokolwiek — gałąź gita przez vc-mode, czas, nazwę projektu przez project-current — cokolwiek co zwraca string.
...
(show-paren-mode -1)
;; Dodatkowo upewnij się, że Flycheck nie podświetla Ci nawiasów jako błędów składniowych:
...
;; Podpowiada klawisze
;; Przykładowo: Naciśnięcie C-c pokaże na dole okno z listą dostępnych skrótów
(use-package which-key
  :ensure t
  :config
  (which-key-mode))

;; Language-Mode zapewnia kolorowanie i podstawowe składnie
;; Python-Mode is on by default
...
;; Linting w czasie rzeczywistym
;; Jak jest w systemie linters to pokaże błędy
(use-package flycheck
  :ensure t
  :init
  (global-flycheck-mode 1))

;; Snippety dla szybkiego wstawiania kodu
;; Daje skróty typu for<TAB> -> wstawia szablon pętli
(use-package yasnippet
  :ensure t
  :config
  (yas-global-mode 1))

;; Code completition framework
(use-package company
  :ensure t
  :init
  (global-company-mode 1)
  :config
  ;; Proste, niezawodne podpowiedzi bez LSP
  ;; - company-dabbrev-code: tylko słowa z kodu (nie z tekstu)
...
;; Autouzupełnianie (używasz już company, więc wystarczy mu prosty backend)
...
;; Bez tego minted (kolorowanie bloków kodu w pdf) nie zadziała
